...
태블로를 어느정도 사용하고 느끼는 점이라면, 코딩을 하지 않고 원하는 그래프를 바로 그릴 수 있다는게 생각보다도 매우 편리했다. 특히 이전처럼 sql과 plotly를 통해 그릴 때에는 쿼리도 작성해야하고, 그래프를 그리기 위한 코드도 작성해야 했다. 심지어 대시보드를 만들고자 할 경우네는 레이아웃도 코드로 작성해야 했기에 매우 불편하고 시간도 많이 걸렸었다.
